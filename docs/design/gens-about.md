...
*Gens* is in early development. The supported toolchain — a purpose-built native Gens runtime and desktop client with an engine-independent, deterministic C# simulation core — is established and recorded in [`docs/engineering/tech-stack.md`](../engineering/tech-stack.md). The deterministic simulation package (partitioned `WorldState`, phased monthly ticks, a command/event envelope, named and persistable PCG32 random streams, canonical `.gens` save serialization with migrations, and a headless campaign bootstrap) is implemented, and on top of it characters and Familia households, land/goods/buildings/villas/labor with a production network, background population groups, the household ledger/market, the action/policy/event/report layer, a native player loop, autonomous rival houses, and dynasty continuity (succession, Regency, the Chronicle) are all implemented and covered by headless exit-gate soak tests, backed by real typed content families in `content/source/` validated against `content/schemas/`. See the [Comprehensive Build Roadmap](../engineering/gens-comprehensive-build-roadmap.md)'s phase checklist for the current, authoritative state of what is and is not built. The Comprehensive Build Roadmap defines the engineering construction order from the current state to a simulation-complete release; the [Feature Roadmap](gens-feature-roadmap.md) defines the corresponding order in which player-facing features and content should come online.
