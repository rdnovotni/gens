...
/// Holding's <see cref="ConstructionQueue"/> head project by one month (gated on <see
/// cref="EstateLookup.HasLaborAvailable"/>, matching <see cref="ConstructionQueue.AdvanceMonth"/>'s
...
/// <see cref="TickPhase.Production"/> writes <c>constructionQueues</c>) so a building finished this
...
public sealed class ConstructionSystem : IMonthlySystem<WorldState>
{
    ...
    public IReadOnlyCollection<string> Reads { get; } = new[] { "constructionQueues", "holdings", "characters" };
    public IReadOnlyCollection<string> Writes { get; } = new[] { "constructionQueues", "buildings", "buildingIds", "eventIds" };
    ...
    public IReadOnlyList<IDomainEvent> Tick(WorldState state, MonthlyTickContext context)
    {
        ...
        // Materialize first: ConstructionQueue is a mutable class mutated in place below, but the
        ...
        var queues = state.ConstructionQueues.InAscendingOrder().ToArray();
...
/// <summary>Emitted whenever a <see cref="ConstructionQueue"/> head project finishes its final month
