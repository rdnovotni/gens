...
internal static unsafe class SdlNative
{
    ...
    internal const ulong WindowResizable = 0x20;
    internal const ulong WindowHighPixelDensity = 0x2000;
    ...
    internal const uint EventWindowFirst = 0x200;
    internal const uint EventWindowLast = 0x2ff;
    internal const uint EventKeyDown = 0x300;
    internal const uint EventKeyUp = 0x301;
    internal const uint EventTextEditing = 0x302;
    ...
    private static IntPtr Resolve(string name, Assembly assembly, DllImportSearchPath? path)
    {
        ...
        string[] candidates = [Path.Combine(AppContext.BaseDirectory, file), Path.Combine(AppContext.BaseDirectory, "runtimes", rid, "native", file)];
        foreach (string candidate in candidates)
            if (File.Exists(candidate) && NativeLibrary.TryLoad(candidate, out IntPtr handle)) return handle;
        throw new DllNotFoundException($"SDL3 native library '{file}' was not found beside the executable or in runtimes/{rid}/native. See experiments/Gens.NativeSpike/README.md.");
    }
    ...
    internal struct Event
    {
        ...
        [FieldOffset(16)] internal WindowEvent Window;
        [FieldOffset(16)] internal KeyboardEvent Key;
        [FieldOffset(16)] internal TextInputEvent Text;
        [FieldOffset(16)] internal MouseMotionEvent Motion;
        [FieldOffset(16)] internal MouseButtonEvent Button;
        [FieldOffset(16)] internal MouseWheelEvent Wheel;
    }
    ...
    [StructLayout(LayoutKind.Sequential)] internal struct KeyboardEvent { internal uint Type; internal uint Reserved; internal ulong Timestamp; internal uint WindowId; internal uint Which; internal uint Scancode; internal uint Key; internal ushort Mod; internal byte Down; internal byte Repeat; }
    [StructLayout(LayoutKind.Sequential)] internal unsafe struct TextInputEvent { internal uint Type; internal uint Reserved; internal ulong Timestamp; internal uint WindowId; internal fixed byte Text[32]; }
    ...
    [StructLayout(LayoutKind.Sequential)] internal struct MouseWheelEvent { internal uint Type; internal uint Reserved; internal ulong Timestamp; internal uint WindowId; internal uint Which; internal float X; internal float Y; internal uint Direction; internal float MouseX; internal float MouseY; }
    ...
    [return: MarshalAs(UnmanagedType.I1)] [DllImport(Library, CallingConvention = CallingConvention.Cdecl)] internal static extern bool SDL_WaitEventTimeout(out Event @event, int timeoutMilliseconds);
